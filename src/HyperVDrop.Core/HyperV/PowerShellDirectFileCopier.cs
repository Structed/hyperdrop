...
{
    private const string ResourceName = "HyperVDrop.Core.HyperV.PowerShellDirectWorker.ps1";
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            throw new HyperVDropException($"\"{request.SourcePath}\" no longer exists on this PC.");
        }
        ...
        CancellationToken cancellationToken)
    {
        while (true)
        {
            ...
            if (line.StartsWith("#E ", StringComparison.Ordinal))
            {
                ...
                throw new HyperVDropException(message, HyperVErrorMessages.RemedyFor(message));
            }

            if (line.StartsWith("#FATAL ", StringComparison.Ordinal))
            {
                throw new HyperVDropException(line[7..].Trim());
            }
        }
    }
    ...
    {
        var worker = _worker ?? throw new HyperVDropException("The PowerShell Direct session is not running.");
    ...
    private async Task<ChannelReader<string>> EnsureWorkerAsync(CancellationToken cancellationToken)
    {
        ...
        var worker = Process.Start(startInfo)
            ?? throw new HyperVDropException("Could not start Windows PowerShell for PowerShell Direct.");
