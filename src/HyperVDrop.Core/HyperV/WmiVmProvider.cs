...
public sealed class WmiVmProvider : IVmProvider
{
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        using var activeSettings = Wmi.FirstRelated(vm, "Msvm_VirtualSystemSettingData", "Msvm_SettingsDefineState")
            ?? throw new HyperVDropException("Hyper-V did not report any settings for this virtual machine.");
    ...
    private static IReadOnlyList<VirtualMachineInfo> List(CancellationToken cancellationToken)
    {
        ...
        {
            throw new HyperVDropException(
                "Hyper-V did not return any virtual machines because HyperVDrop is not running as an administrator.",
                "Close HyperVDrop and start it again as an administrator.");
        }
